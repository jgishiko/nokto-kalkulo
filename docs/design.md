...
- **地の文**: セリフ以外の本文の文字数
- **進捗**: 目標文字数に対する達成率（目標文字数設定時のみ）
- **残り**: 目標達成までの残り文字数（目標文字数設定時のみ）

#### 出力パネルの自動更新とちらつき防止

出力パネルを表示した状態で原稿を編集すると、文字数が自動的に更新されます。ちらつきを防ぐため、以下の最適化を実装しています：

1. **デバウンス処理（1秒）**
   - テキスト入力のたびに更新するのではなく、最後の編集から1秒経過後に更新
   - 連続入力時の無駄な更新処理を削減

2. **差分更新**
   - 前回の出力内容と比較して、変更がある場合のみ再描画
   - 不要な画面更新を回避してちらつきを軽減

3. **非ブロッキング更新**
   - エディタのフォーカスを保持したまま出力パネルを更新
   - 執筆作業を中断させない
...
- `targetWords` - 目標文字数（デフォルト: 0、0の場合は進捗率非表示）
